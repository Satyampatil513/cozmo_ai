...
def main() -> int:
    ...
    report["gates"]["repeatability"] = {
        ...
        "detail": ("checked at the video tier: two walkthroughs of the same property "
                   "(IMG_0460, IMG_0462) agree only on ceiling height, which disagrees by "
                   "18.8 cm (2.830 m vs 3.018 m). Unrepeatable, not repeatable-but-biased. "
                   "Neither clip closes a polygon, so there are no per-wall lengths to compare"),
    }
    report["gates"]["drift_ablation"] = {
        "status": "BUILT",
        "gate": "report states drift handling, with an on/off ablation",
        "detail": ("plane-anchored correction, not 'poses as-is'. On/off ablation is "
                   "benchmark/scripts/ablation.py (benchmark/results/drift_ablation.md): on a "
                   "synthetic two-room flat with injected drift, the shared-wall gap is "
                   "14.1 cm with correction OFF and 0.0 cm ON. No real capture has closed a "
                   "multi-room stitch to run this on (docs/TECHNICAL_REPORT.md sections 4, 7)"),
    }
    report["gates"]["photo_stitch"] = {
        "status": "FAIL",
        "gate": "per-room photo folders -> one stitched plan, footprint +/- 8%",
        "detail": ("stitch_photo_property() runs on the real 5-room set but rejects every "
                   "cross-room edge (implausible camera height, or depth-scale ratio outside "
                   "sanity) - none of that overlap was deliberately shot. Correct refusal, "
                   "but no stitched footprint is produced"),
    }
    report["gates"]["head_to_head"] = {
        "status": "OUT OF SCOPE",
        "gate": "beat or tie an incumbent app on >= 70% of shared dimensions, 2 rooms",
        "detail": "confirmed with the team; no incumbent-app comparison in this submission",
    }
